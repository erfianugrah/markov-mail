...
**Request**:
```bash
curl -X POST https://fraud.erfi.dev/admin/markov/promote-to-canary \
  -H "X-API-Key: $ADMIN_API_KEY" \
...
**Request**:
```bash
curl https://fraud.erfi.dev/admin/markov/canary-metrics \
  -H "X-API-Key: $ADMIN_API_KEY"
```
...
**Request**:
```bash
curl -X POST https://fraud.erfi.dev/admin/markov/promote-to-production \
  -H "X-API-Key: $ADMIN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"version": "B"}'
```
...
**Request**:
```bash
curl -X POST https://fraud.erfi.dev/admin/markov/rollback \
  -H "X-API-Key: $ADMIN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"to_version": "A"}'
```
...
**Request**:
```bash
curl https://fraud.erfi.dev/admin/markov/status \
  -H "X-API-Key: $ADMIN_API_KEY"
```
